...
//has,新添加的两个函数
void CGridCtrl::RowIsEpt(int iRow, int Cols, int& Flag)//0-为空
{
    int i;
    int count = 0;
    for (i = 1; i < Cols; ++i)
...
BOOL CGridCtrl::IsEpt(int i, int j)
{
    CString str;
    BOOL tmp;
    ...
    tmp = str.IsEmpty(); //为空返回true
    return tmp;

}

void CGridCtrl::RowOnlyRead(int nRow)
{
    for (int i = m_nFixedRows; i < m_nRows; ++i)
        SetItemState(nRow, i, GetItemState(nRow, i) | GVIS_READONLY);
}
void CGridCtrl::OnSelChangeCombo()
{
    // TODO: Add your control notification handler code here
}
BOOL CGridCtrl::GetCellType(int nRow, int nCol)
{
    if (GetVirtualMode())
        return FALSE;

    CGridCellCheck* pCheckCell = (CGridCellCheck*)GetCell(nRow, nCol);
    int index = pCheckCell->GetCheck();
    if (index > 0)
        return TRUE;
    else
        return FALSE;
}
BOOL CGridCtrl::SetCellTypeForcheckbox(int nRow, int nCol, BOOL TorF)
{
    if (GetVirtualMode())
        return FALSE;

    CGridCellCheck* pCheckCell = (CGridCellCheck*)GetCell(nRow, nCol);
    int index = pCheckCell->SetCheck(TorF);
